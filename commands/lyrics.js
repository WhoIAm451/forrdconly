...
module.exports = {
  name: "lyrics",
  description: "Lyrics des musiques",
  usage: "[Song Name]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!lyrics)
      return client.sendTime(
        message.channel,
        `**J'ai pas trouvé pour cette musique mec -** \`${SongTitle}\``
      );
    lyrics = lyrics.split("\n"); //spliting into lines
    ...
    let Pages = SplitedLyrics.map((ly) => {
      let em = new MessageEmbed()
        .setAuthor(`Lyrics pour: ${SongTitle}`, client.botconfig.IconURL)
    ...
    options: [
      {
        ...
        value: "song",
        type: 3,
        description: "Entre le titre d'une musique",
        required: false,
      },
    ],
    /**
